...
# based on the config file. Designed to be run by a launchd WatchPaths
# agent (see config/com.CHANGEME.heicwatch.plist.template), but works
# fine run manually or via cron too.
...
convert_one() {
  ...
  if [[ "$FORMAT" == "png" || "$FORMAT" == "both" ]]; then
    ...
    if [[ ! -e "$out_png" ]]; then
      if sips -s format png "$f" --out "$out_png" >> "$LOG" 2>&1; then
  ...
  if [[ "$FORMAT" == "jpg" || "$FORMAT" == "both" ]]; then
    ...
    if [[ ! -e "$out_jpg" ]]; then
      if sips -s format jpeg -s formatOptions "$JPG_QUALITY" "$f" --out "$out_jpg" >> "$LOG" 2>&1; then
